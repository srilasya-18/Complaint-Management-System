...
export default {
  ...
  createComplaint: async (args, req) => {
    ...
    // ── FIX: map uploaded photo metadata into the shape the model expects
    const photos = (args.complaintInput.photos || []).map(p => ({
      url: p.url,
  ...
  // ── existing: view resolved complaint ─────────────────────────────
  ...
  // ── existing: resolveComplaint (backward compat) ───────────────────
